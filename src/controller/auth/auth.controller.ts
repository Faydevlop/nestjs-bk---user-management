...
export class AuthController {
  ...
  @Post('reset-password')
  @ApiOperation({
    summary: 'Reset password using OTP (send OTP first with type PASSWORD)',
  })
  @ApiBody({
  ...
  @Post('send-otp')
  @ApiOperation({
    summary: 'Send OTP — type SIGNUP (with password) or PASSWORD (for reset)',
  })
  @ApiBody({
    type: SendOtpDto,
    examples: {
  ...
  @ApiResponse({
    status: 409,
    description: 'OTP already sent or email conflict.',
  })
  ...
  @ApiOperation({
    summary:
      'Verify OTP — for SIGNUP returns tokens; for PASSWORD returns verified:true',
  })
  @ApiBody({
    type: VerifyOtpDto,
    examples: {
  ...
  @ApiResponse({
    status: 200,
    description: 'Tokens returned (SIGNUP) or verified:true (PASSWORD).',
  })
  @ApiResponse({ status: 400, description: 'Invalid OTP or parameters.' })
  ...
  getProfile(@Request() req: ExpressRequest) {
    return (req as unknown as { user: Record<string, unknown> }).user;
  }
