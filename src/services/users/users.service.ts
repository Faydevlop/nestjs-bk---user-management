...
export class UsersService {
  ...
  async getUsers(
    payload: GetUsersDto,
  ): Promise<{ tableData: UserDocument[]; tableCount: number }> {
      ...
      for (const item of search) {
        ...
        for (const field of item.fields) {
          searchConditions.push({
            [field]: { $regex: new RegExp(regexPattern, 'i') },
          });
        }
      }
    ...
    interface FacetResult {
      tableData: UserDocument[];
      tableCount: Array<{ count: number }>;
    }

    const result: FacetResult[] =
      await this.userModel.aggregate<FacetResult>(pipeline);

    const tableData: UserDocument[] = result[0]?.tableData ?? [];
    const tableCount: number = result[0]?.tableCount?.[0]?.count ?? 0;
